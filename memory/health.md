...
- **✅ LIVE — iOS-Kurzbefehl (gebaut & verifiziert 2026-05-31 15:49):** Ganze Kette läuft end-to-end. Push vom Handy (85.9 kg) landete in `applehealth-weight.json`, Briefing rendert `⚖️ Gewicht: 85.9 kg (Stand 31.05.) ▲ +1.1 kg vs. 29.05.`.
  ...
  - **Automation — GEBAUT & lief automatisch (nach 2026-05-31), dann VERSCHWUNDEN (~2026-06-01):** Nach obigem Stand wurde eine funktionierende Automation gebaut, die den Kurzbefehl automatisch ausführte. Am ~01.06. zeigte Shortcuts plötzlich „Keine Automation" → Push stoppte, `applehealth-weight.json` fror bei 31.05. ein (erst am 04.06. via Briefing bemerkt). Vermutliche Ursache: **iOS hat die Automation (oder den referenzierten Kurzbefehl) gelöscht/deaktiviert** — typisch nach iOS-Update/Restore. NICHT „nie gebaut" (frühere Notiz war veraltet). **Fix:** Automation neu bauen — Automation-Tab → Tageszeit **~07:00 täglich** (MUSS nach Wiegen ~06:23 UND vor dem 07:30-Briefing liegen; „Tageszeit", NICHT „WiFi-Join" — Handy ist nachts schon im WLAN, triggert morgens nicht neu), „Vor dem Ausführen fragen" AUS. Mac-Seite am 04.06. erneut verifiziert: `~/wlog` → Writer voll funktionsfähig, IP unverändert 192.168.1.96.
  - **Safety net (2026-06-04):** `withings-weight.py` Stale-Warnung 7 → **2 Tage** gesenkt (beide Pfade) — eine erneut verschwundene Automation taucht so binnen 1 Tag im Briefing auf statt monatelang stillzustehen.
